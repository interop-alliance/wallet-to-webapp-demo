...
(function () {
  ...
  function copyJSONToClipboard(getJSON) {
    const json =
      (typeof getJSON === 'function' && getJSON()) ||
      getCurrentJSONText() ||
      '';
  ...
  function downloadJSON(getJSON) {
    ...
    const fname = `wallet-result-${new Date()
      .toISOString()
      .replace(/[:.]/g, '-')}.json`;
  ...
  function initActions(opts = {}) {
    ...
    if (copyBtn)
      copyBtn.addEventListener('click', () => copyJSONToClipboard(getJSON));
    if (downloadBtn)
      downloadBtn.addEventListener('click', () => downloadJSON(getJSON));
  }
  ...
  function initZcapActions(opts = {}) {
    ...
    if (zcapCopyBtn)
      zcapCopyBtn.addEventListener('click', () => copyJSONToClipboard(getJSON));
    if (zcapDownloadBtn)
      zcapDownloadBtn.addEventListener('click', () => downloadJSON(getJSON));
  }
  ...
  function initSignRequestActions(opts = {}) {
    ...
    if (signCopyBtn)
      signCopyBtn.addEventListener('click', () => copyJSONToClipboard(getJSON));
    if (signDownloadBtn)
      signDownloadBtn.addEventListener('click', () => downloadJSON(getJSON));
  }
  ...
  window.Actions = {
    ...
    initZcapActions,
    showZcapActions,
    initSignRequestActions,
    showSignRequestActions,
  };
})();
